...
function getCacheKey(playlist) {
  return `${playlist.id}_${playlist.snapshot_id}`;
}

async function loadFromCache(playlist) {
  try {
    const db = await openDB();
    return await new Promise((resolve, reject) => {
      const tx = db.transaction(STORE, 'readonly');
      const req = tx.objectStore(STORE).get(getCacheKey(playlist));
      req.onsuccess = () => {
        const val = req.result;
        if (!val || val.version !== 1) return resolve(null);
        resolve({ items: val.items, albumDates: val.albumDates });
      };
...
async function saveToCache(playlist, items, albumDates) {
  try {
    const db = await openDB();
    await new Promise((resolve, reject) => {
      const tx = db.transaction(STORE, 'readwrite');
      tx.objectStore(STORE).put({ version: 1, items, albumDates }, getCacheKey(playlist));
      tx.oncomplete = resolve;
      tx.onerror = (e) => reject(e.target.error);
    });
    await evictOldCache(db, playlist);
  } catch (e) {
    console.warn('[cache] write failed:', e);
  }
}

async function evictOldCache(db, playlist) {
  return new Promise((resolve) => {
    const tx = db.transaction(STORE, 'readwrite');
    const store = tx.objectStore(STORE);
    const req = store.getAllKeys();
    req.onsuccess = () => {
      const currentKey = getCacheKey(playlist);
      for (const key of req.result) {
        if (key.startsWith(playlist.id + '_') && key !== currentKey) {
          store.delete(key);
        }
      }
      resolve();
...
export async function renderTracks(container, playlist) {
  ...
  const cached = await loadFromCache(playlist);
  if (cached) {
    allItems = cached.items;
    albumDates = cached.albumDates;

    let oldestAddedAt = null;
    for (const item of allItems) {
      const addedAt = validAddedAt(item.added_at);
      if (addedAt && (!oldestAddedAt || addedAt < oldestAddedAt)) oldestAddedAt = addedAt;
    }
    if (oldestAddedAt) {
  ...
  try {
    ...
    saveToCache(playlist, allItems, albumDates);
